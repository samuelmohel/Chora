...
class ReportController extends Controller
{
    ...
    public function accountReceivables(Request $request) {
        if(\Auth::user()->can('bill report')) // confirms user is an admin
        {
    ...
    public function trialBalanceSummary(Request $request, $view = "expand")
    {
        if (\Auth::user()->can('trial balance report')) {
            ...
            foreach ($types as $type) {
                ...
                $totalParentAccountArrays[] = $totalParentAccountArray;
                if ($totalParentAccountArray != []) {
                ...
                if ($totalParentAccountArray != []) {
                    ...
                    // if(isset($totalParentAccountArray[1])) {
                    //     $totalParentAccountArray[0][] = $totalParentAccountArray[1];
                    // }
                    $totalAccount[$name] = array_merge_recursive($totalAccount[$name], $totalParentAccountArray[0]);
                    if(isset($totalParentAccountArray[1])) {
                    $totalAccount[$name] = array_merge_recursive($totalAccount[$name], $totalParentAccountArray[1]);
                    }
                }
            }


            // dd($totalParentAccountArrays);
            // dd($totalAccount);
