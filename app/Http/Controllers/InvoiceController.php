...
class InvoiceController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (\Auth::user()->can('manage invoice')) {
            ...
            $autoInvoice = DB::table('settings')->where('name', 'auto_invoice')->first();

            if (!$autoInvoice) {
                $autoInvoice = DB::table('settings')->insert([
                    'name'       => 'auto_invoice',
                    'value'      => 0,
            ...
            return view('invoice.index', compact('invoices', 'customer', 'status', 'autoInvoice'));
        } else {
    ...
    public function invoiceNumber()
    {
        ...
        $latest = Utility::getValByName('invoice_starting_number', 1);
        if (!$latest) {
            return 1;
        }
        ...
        try {
            if (\Auth::user()->can('show invoice')) {
                ...
                if ($invoice->created_by == \Auth::user()->creatorId()) {
                    ...
                    $customFields         = CustomField::where('created_by', '=', \Auth::user()->creatorId())->where('module', '=', 'invoice')->get();
                    return view('invoice.view', compact('invoice', 'customer', 'iteams', 'invoicePayment', 'customFields', 'users', 'company_payment_setting'));
                } else {
                    return redirect()->back()->with('error', __('Permission denied.'));
                }
            } else {
    ...
    public function import(Request $request)
    {
        ...
        $invoices   = (new InvoiceImport())->toArray($request->file('file'))[0];
        $errorArray = [];

        $starting_number = $this->invoiceNumber();

        for ($i = 1; $i <= count($invoices) - 1; $i++) {
            $invoice  = new Invoice();
            $customer = Customer::where('resident_id', $invoices[$i][0])->first();
            if(!$invoices[$i][3]) {
                continue;
            }
            if (!$customer) {
                array_push($errorArray, "Failed to find resident" . $invoices[$i][0]);
                continue;
            };
            $invoice->customer_id    = $customer->id;
            $invoice->invoice_id     = $starting_number;
            $invoice->issue_date     = Carbon::createFromFormat('Y m d', $invoices[$i][1]);
            $invoice->due_date       = Carbon::createFromFormat('Y m d', $invoices[$i][2]);
            $invoice->discount_apply = 0;
            $invoice->category_id    = 3;
            $invoice->created_by     = \Auth::user()->creatorId();

            $invoice->status    = 1;
            $invoice->send_date = Carbon::now();
            $invoice->save();
            $starting_number++;

            $invoiceProduct              = new InvoiceProduct();
            $invoiceProduct->invoice_id  = $invoice->id;
            $invoiceProduct->product_id  = 1;
            $invoiceProduct->quantity    = 1;
            $invoiceProduct->price       = $invoices[$i][3];
            $invoiceProduct->description = "2024 EOY Outstanding Charges";
            $invoiceProduct->save();
               
            // update customer balance 
            Utility::updateUserBalance('customer', $customer->id, $invoice->getTotal(), 'credit');
        };
        Utility::starting_number($starting_number, 'invoice');

        $errorRecord = [];
        $data        = [];
        if (empty($errorArray)) {
            $data['status'] = 'success';
            $data['msg']    = __('Records successfully imported');
        } else {
            $data['status'] = 'error';
            $data['msg']    = count($errorArray) . ' ' . __('Record imports fail out of' . ' ' . count($invoices) - 1 . ' ' . 'records:::');
    ...
    public function import2024(Request $request)
    {
        ...
        for ($i = 1; $i <= count($invoices) - 1; $i++) {
            // dd($invoices[4]);
            $invoice  = new Invoice();
            $customer = Customer::where('resident_id', $invoices[$i][1])->first();
            if (!$customer || is_null($invoices[$i][0])) {
                array_push($errorArray, "Failed to find resident" . $invoices[$i][1]) . ' ' . $invoices[$i][0];
                continue;
            };
            $invoice->customer_id    = $customer->id;
            $invoice->invoice_id     = $starting_number;
            $invoice->issue_date     = '2024-12-31';
            $invoice->due_date       = '2024-12-31';
            $invoice->discount_apply = 0;
            $invoice->category_id    = 3;
            $invoice->created_by     = \Auth::user()->creatorId();
            ...
            if (isset($invoices[$i][2]) && is_numeric($invoices[$i][2])) {
                // owing ekedc post paid

                $invoiceProduct              = new InvoiceProduct();
                $invoiceProduct->invoice_id  = $invoice->id;
                $invoiceProduct->product_id  = 11;
                $invoiceProduct->quantity    = 1;
                $invoiceProduct->price       = $invoices[$i][2];
                $invoiceProduct->description = "2024 EOY Outstanding EKEDC Post Paid Charges";
                $invoiceProduct->save();
            }

            if (isset($invoices[$i][3]) && is_numeric($invoices[$i][3])) {
                // owing ekedc post paid

                $invoiceProduct              = new InvoiceProduct();
                $invoiceProduct->invoice_id  = $invoice->id;
                $invoiceProduct->product_id  = 7;
                $invoiceProduct->quantity    = 1;
                $invoiceProduct->price       = $invoices[$i][3];
                $invoiceProduct->description = "2024 EOY Outstanding Estate Dues Charges";
                $invoiceProduct->save();
            }

            if (isset($invoices[$i][4]) && is_numeric($invoices[$i][4])) {
                // owing ekedc post paid

                $invoiceProduct              = new InvoiceProduct();
                $invoiceProduct->invoice_id  = $invoice->id;
                $invoiceProduct->product_id  = 12;
                $invoiceProduct->quantity    = 1;
                $invoiceProduct->price       = $invoices[$i][4];
                $invoiceProduct->description = "2024 EOY Outstanding Reconnection Fee Charges";
                $invoiceProduct->save();
            }

            if (isset($invoices[$i][5]) && is_numeric($invoices[$i][5])) {
                // owing ekedc post paid

                $invoiceProduct              = new InvoiceProduct();
                $invoiceProduct->invoice_id  = $invoice->id;
                $invoiceProduct->product_id  = 2;
                $invoiceProduct->quantity    = 1;
                $invoiceProduct->price       = $invoices[$i][5];
                $invoiceProduct->description = "2024 EOY Outstanding Prepaid Meter Charges";
                $invoiceProduct->save();
            }

            if (isset($invoices[$i][6]) && is_numeric($invoices[$i][6])) {
                // owing ekedc post paid

                $invoiceProduct              = new InvoiceProduct();
                $invoiceProduct->invoice_id  = $invoice->id;
                $invoiceProduct->product_id  = 10;
                $invoiceProduct->quantity    = 1;
                $invoiceProduct->price       = $invoices[$i][6];
                $invoiceProduct->description = "2024 EOY Outstanding Fines Charges";
                $invoiceProduct->save();
            }

            if (isset($invoices[$i][7]) && is_numeric($invoices[$i][7])) {
                // owing ekedc post paid

                $invoiceProduct              = new InvoiceProduct();
                $invoiceProduct->invoice_id  = $invoice->id;
                $invoiceProduct->product_id  = 13;
                $invoiceProduct->quantity    = 1;
                $invoiceProduct->price       = $invoices[$i][7];
                $invoiceProduct->description = "2024 EOY Outstanding Meter Replacement Charges";
                $invoiceProduct->save();
            }
    ...
    public function autoInvoice()
    {
        ...
        try {
            if ($autoInvoice->value == 0) {
                DB::table('settings')->where('name', 'auto_invoice')->update([
                    'value' => 1,
                ]);
                $data['status'] = 'success';
                $data['msg']    = 'Auto Invoices turned on';
            } else {
                DB::table('settings')->where('name', 'auto_invoice')->update([
                    'value' => 0,
                ]);
                $data['status'] = 'success';
                $data['msg']    = 'Auto Invoices turned off';
            }
        } catch (Exception $e) {
            $data['status'] = 'error';
            $data['msg']    = 'Error updating auto invoice status';
        }
        return redirect()->back()->with($data['status'], $data['msg']);
    }
    ...
    public function generateMonth()
    {
        ...
        $customers = Customer::where('created_by', \Auth::user()->creatorId())->where('is_occupied', 1)->get();
        // $customers = Customer::all();

        $errors = [];

        $invoice_number = Utility::getValByName('invoice_starting_number', 1);
        foreach ($customers as $customer) {
            // $customer->balance = 0;
            // $customer->save();
            try {
                $customer->generateMonthlyEstateDuesInvoice($invoice_number);  
            
            	// update customer balance 
            // 	Utility::updateUserBalance('customer', $customer->id, 96000, 'credit');
            	Utility::updateUserBalance('customer', $customer->id, 8000, 'credit');
            } catch (Exception $e) {
                $errors[] = $customer->id;
            }
            $invoice_number++;
        }
        Utility::starting_number($invoice_number, 'invoice');

        if (!empty($errors)) {
            return redirect()->back()->with('error', 'Failed to generate invoice for users:: ' . implode(',', $errors));
        } else {
            return redirect()->back()->with('success', 'Monthly invoices generated for all users');
        }
    }
}
