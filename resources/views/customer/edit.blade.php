{{ Form::model($customer, ['route' => ['customer.update', $customer->id], 'method' => 'PUT', 'class' => 'needs-validation', 'novalidate']) }}
<div class="modal-body">
    ...
    <div class="row">
        <div class="col-lg-6 col-md-6 col-sm-6">
            <div class="form-group">
                {{ Form::label('resident_id', __('Resident ID'), ['class' => 'form-label']) }}<x-required></x-required>
                <div class="form-icon-user">
                    {{ Form::text('resident_id', null, ['class' => 'form-control', 'required' => 'required', 'placeholder' => __('Resident ID')]) }}
                </div>
            </div>
        </div>
        <div class="col-lg-6 col-md-6 col-sm-6">
        ...
        <x-mobile div-class="col-md-6" name="contact" label="{{ __('Contact') }}" placeholder="{{ __('Enter Contact') }}"
            required></x-mobile>
        ...
        <div class="col-lg-4 col-md-4 col-sm-6">
            <div class="form-group">
                {{ Form::label('is_occupied', __('Apartment Is Occupied'), ['class' => 'form-label']) }}
                <div class="form-icon-user form-check form-switch custom-switch-v1">
                    <input type="checkbox" name="is_occupied" class="form-check-input input-primary pointer"
                        id="is_occupied_checkbox">
                </div>
            </div>
        </div>
    ...
    <div class="row">
        ...
        <x-mobile div-class="col-md-6" name="billing_phone" label="{{ __('Phone') }}"
            placeholder="{{ __('Enter Phone') }}"></x-mobile>
    ...
    @if (App\Models\Utility::getValByName('shipping_display') == 'on')
        ...
        <div class="row">
            ...
            <x-mobile div-class="col-md-6" name="shipping_phone" label="{{ __('Phone') }}"
                placeholder="{{ __('Enter Phone') }}"></x-mobile>
...
@push('script-page')
    <script>
        $('#is_occupied_checkbox').on('click', function() {
            if ($(this).is(':checked')) {
                $('.modal-body').append($('<input>', {
                    type: 'hidden',
                    val: 'true',
                    name: 'is_occupied',
                    class: 'is_occupied_input'
                }));
            } else {
                $('.is_occupied_input').remove();
            }
        });
    </script>
@endpush
