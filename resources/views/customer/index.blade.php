...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">{{ __('Residents') }}</li>
@endsection

@section('action-btn')
    <div class="d-flex">
        @can('create invoice')
            <a href="#" data-size="md" data-bs-toggle="tooltip" title="Import Resident Balance"
                data-url="{{ route('customer.balance.import') }}" data-ajax-popup="true" data-title="Import Resident Balance"
                class="btn btn-sm btn-primary me-2 d-flex justify-content-center align-content-center">
                Import Balance
                {{-- <i class="ti ti-file-import ms-2 block"></i> --}}
            </a>
        @endcan
                            ...
                            <tbody>
                                @foreach ($customers as $k => $customer)
                                    <tr class="cust_tr" id="cust_detail"
                                        ...
                                        <td class="font-style">{{ $customer['resident_id'] }}</td>
                                        <td class="font-style">{{ $customer['name'] }}</td>
                                        <td>{{ $customer['contact'] }}</td>
                                        <td>{{ $customer['email'] }}</td>
                                        <td class="{{$customer['balance'] <= 0 ? 'text-danger' : 'text-success'}}">{{ \Auth::user()->priceFormat($customer['balance']) }}</td>
                                        <td>{{ $customer['is_occupied'] === 1 ? 'Occupied' : 'Not Occupied' }}</td>
                                        <td>
