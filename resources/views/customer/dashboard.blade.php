...
@endsection
@section('content')
    <div class="row" style="height: 150px; margin-bottom: 32px;">
        <div>
            <div style="box-sizing: border-box; padding: 24px; display: flex; align-items: flex-end;" id="customer_balance"
                class="w-100 h-100 rounded {{ $current_balance <= 0 ? 'bg-danger' : 'bg-success' }}">
                <span
                    style="display: block; width: 100%; text-align: right; color: white; font-size: 48px; line-height: 64px;">{{ \Auth::user()->priceFormat($current_balance) }}</span>
            </div>
        </div>
    </div>
    <div class="row">
